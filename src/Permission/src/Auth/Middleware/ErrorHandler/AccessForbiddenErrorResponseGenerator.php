 ...
 * @package rollun\permission\Auth\Middleware\ErrorHandler
 */
class AccessForbiddenErrorResponseGenerator
{
    /** @var  UrlHelper */
    ...
    public function __invoke(\Throwable $e, Request $request, Response $response)
    {
        if ($e instanceof AccessForbiddenException) {
            $user = $request->getAttribute(
                UserResolver::KEY_ATTRIBUTE_USER,
                ['roles' => [RoleResolver::DEFAULT_ROLE]]
            );
            ...
            if ($e instanceof AlreadyLogginException) {
